...
class Frame(Expr):
    ...
    def __getitem__(self, column_name: str) -> Expr:
        """其实是返回一个Column"""
        return Expr(
            func=lambda df: df[column_name],
            pre=(self, )
        )

    def select(self, *args: "lionqa.Column") -> "Frame":
        cols = [
            f"{col.frame_name}.{col.column_name}" for col in args
        ]
        return Frame(
    ...
    def merge(self, frame: "Frame", on: Expr, how: Literal["left", "right", "inner"]) -> "Frame":
        return Frame(
        ...
        for field, value in list(namespace.items()):
            if field in namespace["__annotations__"]:
                if not isinstance(value, lionqa.Column):
                    raise TypeError("Type annotations can only be used for column definitions")
            elif field == "__name__":
                ...
                for v in value:
                    if not isinstance(v, Constraint):
                        raise TypeError("Elements in __constraints__ must be of type Constraint")
